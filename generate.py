...
def fast_sample_sequence(model, context, length, temperature=1.0, top_k=50, top_p=0.0, device='cpu'):
...
def main(title,subtitle,counts):
    counts=counts+50
    ...
    parser.add_argument('--subtitle', default= subtitle, type=str, required=False, help='生成文章的敘述')
    ...
    samples_file = open(args.save_samples_path + '/'+ title +'.txt', 'w', encoding='utf-8-sig')
    while True:
        if subtitle=="":
            ...
            for _ in range(nsamples // batch_size):
                ...
                for i in range(batch_size):
                    ...
                    text=drop_head(text)
                    text=text.replace('纔','才').replace('瞭','了').replace('嚮','向').replace('隻','只').replace('麵','面').replace('齣','出').replace('傢','家').replace('挫摺','挫折')
                    print(text)
        ...
        #print("=" * 80)
            ...
            for _ in range(nsamples // batch_size):
                ...
                for i in range(batch_size):
                    generated += 1
                    texts = tokenizer.convert_ids_to_tokens(out)
                    ...
                    info = "=" * 40 + " SAMPLE " + str(generated) + " " + "=" * 40 + "\n"                
                    print(info)           
                    texts = ''.join(texts).replace('##', '').strip()
                    texts=drop_back(texts)  
                    texts=texts.replace('纔','才').replace('瞭','了').replace('嚮','向').replace('隻','只').replace('麵','面').replace('齣','出').replace('傢','家').replace('挫摺','挫折')
                    print(texts)
                    #samples_file.write(info)
                    samples_file.write(texts)
                    samples_file.write('\n')
                    #samples_file.write('=' * 90)
                    #samples_file.write('\n' * 2)
            #print("=" * 80)
